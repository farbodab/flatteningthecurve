...
import numpy as np
from app.tools.curvefit.core.utils import data_translator
...
def plot_fits(generator, prediction_times, sharex, sharey, draw_space, plot_obs=None, plot_uncertainty=False):
    ...
    Args:
        generator: (app.tools.curvefit.model_generator.ModelPipeline) that has some draws
        prediction_times: (np.array) of prediction times
        sharex: (bool) fix the x axes
        sharey: (bool) fix the y axes
        draw_space: (callable) which app.tools.curvefit.functions space to plot the draws in
...
def plot_es(pv_group, exp_smoothing, prediction_times, max_last):
    ...
    Args:
        pv_group: (app.tools.curvefit.pv.pv.PVGroup)
