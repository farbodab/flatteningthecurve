...
def predictive_plots_timeline(day):
    modcollab = pd.read_sql_table('predictivemodel', db.engine)
    ...
    # Timeline splice
    on = on[on.accurate_episode_date <= day]
    ...
    div = f.to_json()
    p = Viz.query.filter_by(header="Forecasted Cases Timeline").first()
    p.viz = div
    db.session.add(p)
    db.session.commit()
    return 'success'
